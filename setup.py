...
setup(
    name = "convertvideos",        # what you want to call the archive/egg
    version = version,
    packages=["convertvideos"],    # top-level python modules you can import like
                                #   'import foo'
    dependency_links = [],      # custom links to a specific project
    install_requires=[],
    extras_require={},      # optional features that other packages can require
                            #   like 'helloworld[foo]'
    package_data = {"convertvideos": ["VERSION.txt"]},
    package_dir={'': 'src'},
